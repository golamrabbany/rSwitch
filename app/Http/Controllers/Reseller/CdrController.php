...
class CdrController extends Controller
{
    public function index(Request $request)
    {
        ...
        $this->applyFilters($query, $request, $descendantIds);

        $records = $query->with(['user:id,name', 'sipAccount:id,username'])
            ->orderByDesc('call_start')
            ->paginate(50);

        $stats = $this->getStats($request, $dateFrom, $dateTo, $descendantIds);

        $users = User::whereIn('id', $descendantIds)->select('id', 'name', 'role')->orderBy('name')->get();

        return view('reseller.cdr.index', compact(
    ...
    public function show(Request $request, string $uuid)
    {
        ...
        return view('reseller.cdr.show', compact('record'));
    }
    ...
    public function export(Request $request)
    {
        ...
        $callback = function () use ($query) {
            ...
            $query->with(['user:id,name', 'sipAccount:id,username'])
                ->orderBy('call_start')
    ...
    private function applyFilters($query, Request $request, array $descendantIds): void
    {
    ...
    private function getStats(Request $request, Carbon $dateFrom, Carbon $dateTo, array $descendantIds): array
    {
        ...
        if ($canUseSummary) {
            ...
            $row = $summaryQuery->selectRaw('
                ...
                COALESCE(SUM(total_reseller_cost), 0) as total_cost
            ')->first();
        ...
        $this->applyFilters($statsQuery, $request, $descendantIds);
