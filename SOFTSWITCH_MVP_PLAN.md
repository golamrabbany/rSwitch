...
├── codec_allow VARCHAR(100)
├── max_channels INT DEFAULT 30
├── prefix VARCHAR(10) NULL         -- tech prefix to prepend
├── strip_digits INT DEFAULT 0      -- digits to strip from dialed number
├── status ENUM('active','disabled')
├── priority INT DEFAULT 1          -- for failover ordering
├── created_at, updated_at
```
...
├── provider VARCHAR(100)
├── trunk_id (FK→trunks.id)         -- inbound trunk
├── assigned_to_user_id (FK→users.id) NULL  -- client or reseller
├── destination_type ENUM('sip_account','ivr','queue','external')
...
; Realtime endpoints are loaded dynamically from MySQL
; Static trunk definitions can go here or also in realtime
```
...
```ini
; === INBOUND from SIP accounts (outbound calls) ===
[from-internal]
; All registered SIP endpoints land here
exten => _X.,1,NoOp(Outbound call from ${CALLERID(num)} to ${EXTEN})
 ...
 ; AGI returns: RATE, MAX_DURATION, ALLOWED (yes/no)
 same => n,GotoIf($["${AGIRESULT}" = "DENIED"]?denied)
 same => n,Set(CDR(userfield)=${RATE_ID})
 same => n,Set(TIMEOUT(absolute)=${MAX_DURATION})
 same => n,Dial(PJSIP/${EXTEN}@trunk-primary,60,T)
 same => n,GotoIf($["${DIALSTATUS}" = "BUSY"]?busy:hangup)
...
; === INBOUND from PSTN trunks ===
[from-trunk]
exten => _X.,1,NoOp(Inbound call to DID ${EXTEN})
 same => n,AGI(agi://127.0.0.1:4573,route_inbound,${EXTEN})
 ; AGI looks up DID→destination mapping, returns endpoint to dial
...
1. check_balance(endpoint, destination)
   ...
   - Return ALLOWED/DENIED + max_duration + rate info

2. route_inbound(did_number)
   - Look up DID → destination mapping
   - Return destination type and endpoint
```
...
- Trunk management (Admin: add/edit/delete outbound carriers)
- Basic dialplan (outbound calls via trunks, inbound routing)
- DID management and assignment to SIP accounts
- SIP registration monitoring via AMI
- CDR collection via `cdr_adaptive_odbc` into MySQL
- Basic CDR viewer (raw, unrated)
- **Deliverable**: Working calls — SIP accounts can register, make/receive calls
...
| Trunk failover | Priority-based in dialplan | Simple; effective for MVP |
