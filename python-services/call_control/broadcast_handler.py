...
def _lookup_rate(session, user_id: int, destination: str, rate_type: str):
    """Longest prefix match with rate_type."""
    user = session.execute(
        text("SELECT rate_group_id FROM users WHERE id = :id"),
        {"id": user_id},
    ).first()
    ...
    return session.execute(
        ...
        """),
        {"rg": user.rate_group_id, "dest": destination, "rt": rate_type},
    ).first()


def _commit_broadcast_call(session, *, broadcast_id: int, number_id: int,
                           user_id: int, sip_account_id, callee: str,
                           caller_id: str, call_start: datetime, call_end: datetime,
                           duration: int, survey_response, destination: str) -> None:
    """Atomic transaction: rate-lookup → balance deduct → CDR insert →
    broadcast_numbers update → broadcast counter update.

    All five statements share the session yielded by get_session(), which
    commits on success or rolls back on any exception. Splitting them
    across multiple db_thread calls would lose atomicity — a crash mid-flow
    could leave the user balance debited but the CDR or counters missing.
    """
    rate = _lookup_rate(session, user_id, callee, "broadcast")
    if not rate:
        rate = _lookup_rate(session, user_id, callee, "regular")
    ...
    if cost > 0:
        session.execute(
            text("UPDATE users SET balance = balance - :cost WHERE id = :uid"),
    ...
    logger.info(
        f"Broadcast call committed: broadcast={broadcast_id} number={number_id} "
        f"duration={duration}s cost={cost} survey={survey_response}"
    )
...
class BroadcastCallHandler:
    ...
    Sync DB calls run off-loop via shared.database.db_thread. The five
    DB statements in _commit_broadcast_call run as one atomic transaction
    inside a single db_thread call.
    """

    async def handle(self, agi):
        try:
            ...
            user_id_raw = await agi.get_variable("RSWITCH_USER_ID")
            sip_account_id_raw = await agi.get_variable("RSWITCH_SIP_ACCOUNT_ID")
            ...
            sip_account_id = int(sip_account_id_raw) if sip_account_id_raw else None

            call_start = datetime.utcnow()
            survey_response = await self._play_and_collect(
                agi, broadcast_type, voice_file
            )

            call_end = datetime.utcnow()
            duration = max(int((call_end - call_start).total_seconds()), 1)

            await db_thread(lambda s: _commit_broadcast_call(
                s,
    ...
    async def _play_and_collect(self, agi, broadcast_type: str, voice_file: str):
        """Stream prompt(s) and (for survey) collect DTMF. Returns the
        survey_response JSON string (or None for simple broadcast / no-answer).
        Wrapped exceptions are downgraded — the caller still bills the call.
        """
