    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-3">
        <form method="GET" class="space-y-3">
            <div style="display: grid; grid-template-columns: 1fr 1fr auto auto auto; gap: 0.75rem; align-items: center;">
                ...
                <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                    <a href="{{ route('admin.operational-reports.hourly', array_merge(request()->except('call_type'), [])) }}" class="px-3 py-2 text-sm font-medium {{ !request('call_type') ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' }}">All</a>
                    <a href="{{ route('admin.operational-reports.hourly', array_merge(request()->except('call_type'), ['call_type' => 'regular'])) }}" class="px-3 py-2 text-sm font-medium border-l {{ request('call_type') === 'regular' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' }}">Regular</a>
                    <a href="{{ route('admin.operational-reports.hourly', array_merge(request()->except('call_type'), ['call_type' => 'broadcast'])) }}" class="px-3 py-2 text-sm font-medium border-l {{ request('call_type') === 'broadcast' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' }}">Broadcast</a>
                </div>
