<x-admin-layout>
    ...
    <div class="filter-card mb-3">
        <form method="GET" class="space-y-3">
            {{-- Row 1: Date + Trunk + Direction + Disposition --}}
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden text-xs flex-shrink-0">
                    ...
                    <a href="{{ route('admin.operational-reports.hourly', array_merge(request()->except('disposition'), ['disposition' => 'ANSWERED'])) }}" class="px-3 py-2 font-medium border-l {{ request('disposition') === 'ANSWERED' ? 'bg-emerald-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' }}">Answered</a>
                    <a href="{{ route('admin.operational-reports.hourly', array_merge(request()->except('disposition'), ['disposition' => 'NO ANSWER'])) }}" class="px-3 py-2 font-medium border-l {{ request('disposition') === 'NO ANSWER' ? 'bg-amber-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' }}">No Answer</a>
                    <a href="{{ route('admin.operational-reports.hourly', array_merge(request()->except('disposition'), ['disposition' => 'FAILED'])) }}" class="px-3 py-2 font-medium border-l {{ request('disposition') === 'FAILED' ? 'bg-red-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' }}">Failed</a>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                @if(request()->hasAny(['reseller_id', 'user_id', 'trunk_id', 'disposition', 'call_flow', 'call_type', 'date_from', 'date_to']))
                    <a href="{{ route('admin.operational-reports.hourly') }}" class="btn-clear">Clear</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200">
                        ...
                        <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Hour</th>
                ...
                <tbody>
                    @foreach($rows as $index => $row)
                        <tr class="{{ $loop->even ? 'bg-gray-50/50' : 'bg-white' }} hover:bg-indigo-50/50 transition-all border-b border-gray-100 group {{ $row->total_calls === 0 ? 'text-gray-400' : '' }}">
                            <td class="px-3 py-2 text-gray-400 tabular-nums text-center">{{ $index + 1 }}</td>
                            <td class="px-3 py-2 font-medium {{ $row->total_calls > 0 ? 'text-gray-900' : '' }}">{{ $row->hour_label }}</td>
                            <td class="px-3 py-2 text-right tabular-nums {{ $row->total_calls > 0 ? 'font-medium' : '' }}">{{ number_format($row->total_calls) }}</td>
                            <td class="px-3 py-2 text-right tabular-nums {{ $row->answered_calls > 0 ? 'text-emerald-600' : '' }}">{{ number_format($row->answered_calls) }}</td>
                            <td class="px-3 py-2 text-right tabular-nums {{ $row->failed_calls > 0 ? 'text-red-500' : '' }}">{{ number_format($row->failed_calls) }}</td>
