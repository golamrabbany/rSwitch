    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-6" x-data="clientSearch()">
        <form method="GET" class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                <a href="{{ route('admin.operational-reports.daily', array_merge(request()->except('disposition'), [])) }}"
                ...
                <a href="{{ route('admin.operational-reports.daily', array_merge(request()->except('disposition'), ['disposition' => 'ANSWERED'])) }}"
                ...
                <a href="{{ route('admin.operational-reports.daily', array_merge(request()->except('disposition'), ['disposition' => 'NO ANSWER'])) }}"
                ...
                <a href="{{ route('admin.operational-reports.daily', array_merge(request()->except('disposition'), ['disposition' => 'FAILED'])) }}"
            ...
            @if(request()->hasAny(['reseller_id', 'user_id', 'trunk_id', 'disposition', 'date_from', 'date_to']))
                <a href="{{ route('admin.operational-reports.daily') }}" class="px-3 py-2 text-sm text-gray-500 hover:text-gray-700">Clear</a>
            @endif
        </form>
    </div>
                    ...
                    <tbody>
                        @foreach($rows as $index => $row)
                            <tr>
                                ...
                                <td class="font-medium text-gray-900">{{ \Carbon\Carbon::parse($row->date)->format('M d, Y') }}</td>
