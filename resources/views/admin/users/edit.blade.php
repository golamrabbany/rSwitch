    ...
    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6">
            ...
            <div class="bg-white shadow sm:rounded-lg p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="number" id="credit_limit" name="credit_limit" value="{{ old('credit_limit', $user->credit_limit) }}" step="0.01" min="0"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    <div>
                        ...
                        <input type="number" id="max_channels" name="max_channels" value="{{ old('max_channels', $user->max_channels) }}" min="1"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        <x-input-error :messages="$errors->get('max_channels')" class="mt-2" />
                    </div>
                    <div>
                        <label for="daily_spend_limit" class="block text-sm font-medium text-gray-700">Daily Spend Limit</label>
                        <input type="number" id="daily_spend_limit" name="daily_spend_limit" value="{{ old('daily_spend_limit', $user->daily_spend_limit) }}" step="0.01" min="0"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    ...
                    <div>
                        <label for="daily_call_limit" class="block text-sm font-medium text-gray-700">Daily Call Limit</label>
                        <input type="number" id="daily_call_limit" name="daily_call_limit" value="{{ old('daily_call_limit', $user->daily_call_limit) }}" min="0"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        <p class="mt-1 text-xs text-gray-500">Leave empty for unlimited</p>
            ...
            <div class="flex items-center justify-end gap-x-3">
                <a href="{{ route('admin.users.show', $user) }}" class="text-sm font-semibold text-gray-900">Cancel</a>
