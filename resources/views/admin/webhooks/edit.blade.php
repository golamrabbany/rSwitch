        ...
        <div class="bg-white shadow sm:rounded-lg p-6">
            <form method="POST" action="{{ route('admin.webhooks.update', $webhook) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="url" id="url" name="url" value="{{ old('url', $webhook->url) }}" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                ...
                <div>
                    ...
                    <input type="text" id="description" name="description" value="{{ old('description', $webhook->description) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                ...
                <div class="flex justify-end gap-3">
                    <a href="{{ route('admin.webhooks.show', $webhook) }}" class="rounded-md bg-white px-4 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">Cancel</a>
                    <button type="submit" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">Update Endpoint</button>
                </div>
            </form>
        </div>
    </div>
