    ...
    <div class="mb-6 bg-white shadow sm:rounded-lg p-4">
        <form method="GET" action="{{ route('admin.cdr.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div>
                    <label for="call_flow" class="block text-xs font-medium text-gray-500 mb-1">Call Flow</label>
                    <select id="call_flow" name="call_flow"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                ...
                <div>
                    <label for="trunk_id" class="block text-xs font-medium text-gray-500 mb-1">Trunk</label>
                    <select id="trunk_id" name="trunk_id"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        <option value="">All Trunks</option>
                        @foreach ($trunks as $trunk)
                            <option value="{{ $trunk->id }}" {{ request('trunk_id') == $trunk->id ? 'selected' : '' }}>
                                {{ $trunk->name }} ({{ $trunk->direction }})
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="flex items-center gap-3 mt-4">
                ...
                <a href="{{ route('admin.cdr.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
                <a href="{{ route('admin.cdr.export', request()->query()) }}"
                ...
                @forelse ($records as $record)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-sm text-right">
                            <a href="{{ route('admin.cdr.show', ['uuid' => $record->uuid, 'date' => $record->call_start?->format('Y-m-d')]) }}"
