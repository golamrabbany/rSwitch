        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="form-card">
                <div class="form-card-header">
                    ...
                    <p class="form-card-subtitle">Block numbers from all broadcasts</p>
                </div>
            ...
            <div class="detail-card">
                <div class="detail-card-header">
                    <h3 class="detail-card-title">Summary</h3>
                </div>
        ...
        <div class="lg:col-span-2">
            ...
            <div class="filter-card mb-3">
                <form method="GET" class="filter-row">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.dnc.index') }}" class="btn-clear">Clear</a>
                    @endif
                </form>
            </div>
            ...
            <div x-data="{ selected: [], selectAll: false }" class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                @if($numbers->total() > 0)
                    <div class="px-4 py-2 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                        ...
                        {{-- Bulk Delete --}}
                        <form x-show="selected.length > 0" x-cloak method="POST" action="{{ route('admin.dnc.bulk-destroy') }}" onsubmit="return confirm('Remove selected numbers from DNC list?')">
                            @csrf
                    ...
                    <tbody>
                        @forelse ($numbers as $dnc)
                            <tr class="{{ $loop->even ? 'bg-gray-50/50' : 'bg-white' }} hover:bg-indigo-50/50 transition-all border-b border-gray-100 group">
                                ...
                                <td class="px-3 py-2">
                                    <span class="font-mono font-semibold text-gray-900">{{ $dnc->phone_number }}</span>
                                </td>
                                <td class="px-3 py-2 text-gray-500">{{ $dnc->reason ?: '—' }}</td>
                                <td class="px-3 py-2">
                                    <span class="text-sm text-gray-700">{{ $dnc->addedBy?->name ?? '—' }}</span>
                                </td>
                                <td class="px-3 py-2 text-gray-500">{{ $dnc->created_at?->format('d M Y') }}</td>
