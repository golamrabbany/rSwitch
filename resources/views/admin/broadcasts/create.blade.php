                                ...
                                <template x-for="(q, qIdx) in surveyQuestions" :key="qIdx">
                                    <div class="border border-gray-200 rounded-lg p-4 space-y-3" :class="q.type === 'intro' ? 'bg-blue-50/50 border-blue-200' : 'bg-white'">
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <button type="button" @click="removeQuestion(qIdx)" class="p-1 text-red-400 hover:text-red-600 rounded transition-colors"
                                                    x-show="q.type === 'intro' || surveyQuestions.filter(s => s.type === 'question').length > 1">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select :name="'survey_questions[' + qIdx + '][voice_file_id]'" x-model="q.voice_file_id" class="form-select text-sm" required>
                                                <option value="">Select voice file...</option>
                                                <template x-for="vf in voiceFiles" :key="vf.id">
                                                    <option :value="vf.id" x-text="vf.name + ' (' + (vf.duration || '?') + 's)'"></option>
                                                </template>
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" :name="'survey_questions[' + qIdx + '][label]'" x-model="q.label" class="form-input text-sm"
                                                   :placeholder="q.type === 'intro' ? 'e.g. Welcome to our survey' : 'e.g. How satisfied are you?'">
                                        </div>

                                        {{-- Question-specific fields (not for intro) --}}
                                        <template x-if="q.type === 'question'">
                                            <div class="space-y-3">
                                                {{-- Settings Row --}}
                                                <div class="flex gap-3">
                                                    <div class="form-group" style="flex:0 0 100px;">
                                                        <label class="form-label text-xs">Max Digits</label>
                                                        <input type="number" :name="'survey_questions[' + qIdx + '][max_digits]'" x-model="q.max_digits" min="1" max="10" class="form-input text-sm">
                                                    </div>
                                                    <div class="form-group" style="flex:0 0 100px;">
                                                        <label class="form-label text-xs">Timeout (s)</label>
                                                        <input type="number" :name="'survey_questions[' + qIdx + '][timeout]'" x-model="q.timeout" min="3" max="60" class="form-input text-sm">
                                                    </div>
                                                    <div class="form-group" style="flex:0 0 100px;">
                                                        <label class="form-label text-xs">Retries</label>
                                                        <input type="number" :name="'survey_questions[' + qIdx + '][max_retries]'" x-model="q.max_retries" min="0" max="5" class="form-input text-sm">
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="space-y-1.5">
                                                        <template x-for="(opt, oIdx) in q.options" :key="oIdx">
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                <input type="text" :name="'survey_questions[' + qIdx + '][options][' + oIdx + '][label]'" x-model="opt.label" class="form-input text-sm" placeholder="Label (e.g. Satisfied)">
                                                                <button type="button" @click="removeOption(qIdx, oIdx)" class="p-1 text-red-400 hover:text-red-600 rounded" x-show="q.options.length > 1">
