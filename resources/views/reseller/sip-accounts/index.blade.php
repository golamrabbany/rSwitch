<x-reseller-layout>
            ...
            <tbody>
                @forelse ($sipAccounts as $sip)
                    <tr class="{{ $loop->even ? 'bg-gray-50/50' : 'bg-white' }} hover:bg-emerald-50/50 transition-all border-b border-gray-100 group">
                        ...
                        <td class="px-3 py-2">
                            <a href="{{ route('reseller.clients.show', $sip->user) }}" class="font-medium text-emerald-600 hover:text-emerald-700">{{ $sip->user->name }}</a>
                            <div class="text-xs text-gray-500">{{ $sip->user->email }}</div>
                        </td>
                        <td class="px-3 py-2">
                        ...
                        <td class="px-3 py-2 text-center">
                            <div class="flex items-center justify-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('reseller.sip-accounts.show', $sip) }}" class="p-1.5 rounded-lg text-blue-500 hover:text-blue-700 hover:bg-blue-50 transition-colors" title="View">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-4 py-12 text-center">
                            ...
                            <p class="text-sm text-gray-400">No SIP accounts found</p>
                            <button @click="openAdd()" class="text-sm text-emerald-600 hover:text-emerald-700 font-medium mt-1">Create your first SIP account</button>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if($sipAccounts->hasPages())
        <div class="mt-4 flex justify-end">{{ $sipAccounts->withQueryString()->links() }}</div>
    @endif
                                ...
                                <div>
                                    ...
                                    @if(!empty(auth()->user()->sip_ranges))
                                        @foreach(auth()->user()->sip_ranges as $range)
                                            <p class="text-xs text-emerald-500 mt-0.5 font-medium">Range: {{ $range['start'] }} — {{ $range['end'] }}</p>
                                        @endforeach
                                    @else
...
function sipPage() {
    return {
        ...
        openEdit(data) {
            ...
            if (row) this.editUsername = row.querySelector('.font-mono.font-medium')?.textContent?.trim() || '';
            this.form = {
