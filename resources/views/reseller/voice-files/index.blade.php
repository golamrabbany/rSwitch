<x-reseller-layout>
    ...
    <div class="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <a href="{{ route('reseller.voice-files.index', ['status' => 'draft']) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border {{ request('status') === 'draft' ? 'bg-gray-100 border-gray-300 text-gray-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50' }} text-sm font-medium transition-colors">
            ...
            <tbody>
                @forelse ($voiceFiles as $file)
                    <tr class="{{ $loop->even ? 'bg-gray-50/50' : 'bg-white' }} hover:bg-emerald-50/50 transition-all border-b border-gray-100 group">
                        ...
                        <td class="px-3 py-2">
                            <p class="text-sm font-medium text-gray-800">{{ $file->user?->name ?? '—' }}</p>
                            <p class="text-xs text-gray-400">{{ $file->user?->email ?? '' }}</p>
                        </td>
                        ...
                        <td class="px-3 py-2 text-center">
                            <div class="flex items-center justify-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                                ...
                                @if(in_array($file->status, ['draft', 'pending']))
                                ...
                                @else
                                    <span class="p-1.5 rounded-lg text-gray-300 cursor-not-allowed" title="{{ ucfirst($file->status) }} — cannot edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </span>
                                @endif
                            </div>
