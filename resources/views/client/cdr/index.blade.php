<x-client-layout>
    ...
    <div class="mb-6 bg-white shadow sm:rounded-lg p-4">
        <form method="GET" action="{{ route('client.cdr.index') }}">
            ...
            <div class="flex items-center gap-3 mt-4">
                ...
                <a href="{{ route('client.cdr.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
                <a href="{{ route('client.cdr.export', request()->query()) }}"
                   class="ml-auto rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">Export CSV</a>
            </div>
        </form>
    </div>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($records as $record)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ $record->call_start?->format('M d H:i:s') }}</td>
                        <td class="px-4 py-3 text-sm font-mono text-gray-900">{{ $record->caller }}</td>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-900 text-right tabular-nums">${{ number_format($record->total_cost, 4) }}</td>
                        <td class="px-4 py-3 text-sm">
                            @switch($record->disposition)
                                ...
                                @case('FAILED')
                                    ...
                                    @break
                                @default
                                    <span class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-800">{{ $record->disposition }}</span>
                            @endswitch
                        </td>
                        <td class="px-4 py-3 text-sm text-right">
                            <a href="{{ route('client.cdr.show', ['uuid' => $record->uuid, 'date' => $record->call_start?->format('Y-m-d')]) }}"
                               class="text-indigo-600 hover:text-indigo-500">View</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500">No call records found for this date range.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
