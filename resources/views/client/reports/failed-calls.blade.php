<x-client-layout>
    ...
    <div class="page-header-row">
        <div>
            ...
            <p class="page-subtitle">Unanswered calls — {{ $dateFrom->format('M d') }} to {{ $dateTo->format('M d, Y') }}</p>
        </div>
        <div class="page-actions">
            <a href="{{ route('client.reports.export-failed-calls', request()->query()) }}" class="btn-action-secondary">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">Total Failed</p>
            <p class="text-2xl font-bold text-red-500">{{ number_format($stats['total']) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">Ring Time</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="text-lg font-bold text-gray-900">{{ $dateFrom->format('M d') }} — {{ $dateTo->format('M d') }}</p>
        </div>
    </div>
    ...
    <div class="filter-card mb-3">
        <form method="GET" class="filter-row flex-wrap">
            ...
            @if(request()->hasAny(['search', 'date_from', 'date_to']))
                <a href="{{ route('client.reports.failed-calls') }}" class="btn-clear">Clear</a>
            @endif
        </form>
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-200">
                    ...
                    <th class="px-3 py-2 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Ring (s)</th>
            ...
            <tbody>
                @forelse ($records as $r)
                    <tr class="{{ $loop->even ? 'bg-gray-50/50' : 'bg-white' }} hover:bg-indigo-50/50 transition-all border-b border-gray-100">
                        ...
                        <td class="px-3 py-2 text-right tabular-nums text-gray-600">{{ $r->duration }}</td>
                        <td class="px-3 py-2">
                            @switch($r->disposition)
                                ...
                                @default
                                    <span class="inline-flex items-center gap-1 text-xs font-medium text-gray-500"><span class="w-1.5 h-1.5 rounded-full bg-gray-400"></span>{{ $r->disposition }}</span>
                            @endswitch
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center">
                            ...
                            <p class="text-sm text-gray-400">No failed calls for this period</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
