    ...
    @if($rateGroup)
        <div class="page-header-row">
            <div>
                ...
                <p class="page-subtitle">Your call rate plan &middot; {{ number_format($rates->total()) }} rates &middot; Assigned by Reseller</p>
            </div>
        </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-4 py-12 text-center">
                                ...
                                <p class="text-sm text-gray-400">{{ request('search') ? 'No rates found for "' . request('search') . '"' : 'No rates in this rate group' }}</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($rates->hasPages())
            <div class="mt-4 flex justify-end">{{ $rates->withQueryString()->links() }}</div>
        @endif
    @else
        <div class="page-header-row">
            <div>
                <h2 class="page-title">Base Rate</h2>
                <p class="page-subtitle">Your call rate plan</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 py-16 text-center">
            <svg class="w-12 h-12 text-amber-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
