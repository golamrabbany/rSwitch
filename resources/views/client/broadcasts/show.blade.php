<x-client-layout>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4 mb-6">
        ...
        <div class="w-full bg-gray-200 rounded-full h-3">
            <div class="bg-indigo-500 h-3 rounded-full transition-all duration-500" style="width: {{ $broadcast->total_numbers > 0 ? round($broadcast->dialed_count / $broadcast->total_numbers * 100) : 0 }}%"></div>
        </div>
    ...
    <div class="flex items-center gap-3 mb-6">
        @if($broadcast->status === 'draft')
            <form method="POST" action="{{ route('client.broadcasts.start', $broadcast) }}">
                @csrf
        ...
        @if($broadcast->status === 'running')
            <form method="POST" action="{{ route('client.broadcasts.pause', $broadcast) }}">
                @csrf
        ...
        @if($broadcast->status === 'paused')
            <form method="POST" action="{{ route('client.broadcasts.resume', $broadcast) }}">
                @csrf
                <button type="submit" class="btn-primary">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/>
        ...
        @if(in_array($broadcast->status, ['draft', 'scheduled', 'queued', 'running', 'paused']))
            <form method="POST" action="{{ route('client.broadcasts.cancel', $broadcast) }}" onsubmit="return confirm('Are you sure you want to cancel this broadcast?')">
                @csrf
        ...
        @if(in_array($broadcast->status, ['running', 'completed', 'paused']))
            <a href="{{ route('client.broadcasts.results', $broadcast) }}" class="btn-action-secondary">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
