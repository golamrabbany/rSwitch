...
install_python_services() {
    ...
    $PY_MYSQL -e "
        ...
        GRANT UPDATE (last_registered_at, last_registered_ip) ON ${DB_NAME}.sip_accounts TO 'python_svc'@'localhost';
        FLUSH PRIVILEGES;
...
server {
    ...
    # rSwitch: live-calls WebSocket -> Python engine (FastAPI on :8001)
    location /ws/ {
